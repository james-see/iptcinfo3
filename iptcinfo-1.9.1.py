...
class IPTCData(dict):
  ...
  def keyAsInt(self, key):
    global c_datasets_r
    if isinstance(key, int) and c_datasets.has_key(key): return key
...
class IPTCInfo(object):
  ...
  def __init__(self, filename, force=False, *args, **kwds):
    super(type(self), self).__init__(self, *args, **kwds)
    # Open file and snarf data from it.
    self.error = None
    self._fh = None
    fh = file(filename, 'rb')
    if not fh: raise Exception("Can't open file!")
    ...
    datafound = self.scanToFirstIMMTag()
  ...
  def readExactly(self, length):
    ...
    if buf is None or len(buf) < length: raise EOFException
    return buf
    
  def seekExactly(self, length):
    """seekExactly
    
    Seeks length bytes from the current position and checks the result
    """
    pos = self._fh.tell()
    self._fh.seek(length, 1)
    if self._fh.tell() - pos != length: raise EOFException()
  ...
  def scanToFirstIMMTag(self): #OK#
    ...
    if self.fileIsJpeg():
      ...
      return self.jpegScan()
    else:
      self.log("File not a JPEG, trying BlindScan")
      return self.blindScan()

  def fileIsJpeg(self): #OK#
    ...
    if not self._fh: self._fh = file(self._filename, 'rb')
    self._fh.seek(0, 0)
    if debugMode > 0:
      self.log("Opening 16 bytes of file:\n");
      dump = self._fh.read(16)
      debug(3, self.hexDump(dump))
      self._fh.seek(0, 0)
    # check start of file marker
    ered = False
    try: 
      (ff, soi) = self._fh.read(2)
  ...
  def jpegScan(self): #OK#
    ...
    return self.blindScan(self.jpegGetVariableLength())

  def jpegNextMarker(self): #OK#
  ...
  def jpegGetVariableLength(self): #OK#
    ...
    length field."""
    try: length = unpack('!H', self.readExactly(2))[0]
  ...
  def jpegSkipVariable(self, rSave=None): #OK#
    ...
    length = self.jpegGetVariableLength()
    if length == 0: return None

    # Skip remaining bytes
    if rSave is not None or debugMode > 0:
      try: temp = self.readExactly(length)
      except EOFException:
  ...
  def blindScan(self, MAX=8192): #OK#
    ...
    while offset <= MAX:
      ...
      if ord(temp) == 0x1c:
        ...
        (record, dataset) = self._fh.read(2)
        if ord(record) == 1 and ord(dataset) == 90:
          # found character set's record!
          try: 
            temp = self.readExactly(self.jpegGetVariableLength())
          ...
          try: self.seekExactly(-3) # seek rel to current position
          except EOFException:
            return None
          return offset
        else:
          ...
          # those reads above.
          try: self.seekExactly(-2) # seek rel to current position
          except EOFException: return None
  ...
  def jpegCollectFileParts(self, discardAppParts=False):
    ...
    marker = self.jpegNextMarker()
    app0data = ''
    app0data = self.jpegSkipVariable(app0data)
    if app0data is None:
    ...
    while 1:
      ...
      # Check for start of compressed data
        ...
        break
      partdata = ''
      partdata = self.jpegSkipVariable(partdata)
      if not partdata:
        self.error = "JpegSkipVariable failed"
        self.log(self.error)
        return None
